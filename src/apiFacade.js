...
function apiFacade() {
  ...
  const postNewBoat = (name, brand, make, year, image, auctionid) => {
    const options = makeOptions("POST", true, {
          name: name,
          brand: brand,
          make: make,
    ...
    return fetch(URL + "/api/boat/addnewtoauc/" + auctionid, options).then(handleHttpErrors);
  };
...
return fetch(URL + "/api/boat/updateboat", options).then(handleHttpErrors);
  }
